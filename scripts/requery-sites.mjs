...
const siteLocs = JSON.parse(
  await readFile(
    "research/ownership-parcellation/data/site-locations.json",
    "utf8",
  ),
);

// WGS84 → ITM conversion (calibrated from parcels GeoJSON)
const wgsMinLon = 34.77883,
  wgsMinLat = 32.03474;
const itmMinX = 179800,
  itmMinY = 661200;
...
function wgsToItm(lon, lat) {
  return [
    Math.round(itmMinX + (lon - wgsMinLon) * sx),
    Math.round(itmMinY + (lat - wgsMinLat) * sy),
  ];
}
...
for (const siteId of siteIds) {
  ...
  for (const parcel of parcelsGJ.features) {
    ...
    for (const permit of permitsGJ.features) {
      if (!permit.properties.addresses || !permit.geometry?.coordinates)
        continue;
  ...
  const withAddr = Object.values(addrResult[siteId]).filter(
    (a) => a.length > 0,
  ).length;
  console.log(
    `${siteId}: ${withAddr}/${parcelsGJ.features.length} parcels with addresses`,
  );
}

await writeFile(
  "research/ownership-parcellation/data/_parcel_addresses.json",
  JSON.stringify(addrResult, null, 2),
);
