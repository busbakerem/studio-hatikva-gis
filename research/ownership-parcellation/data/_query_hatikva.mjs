...
await writeFile(
  base + "/municipal_land.geojson",
  JSON.stringify(municipal, null, 2),
);
...
const stats = {
  municipal: {
    ...
    fields: municipal.features[0]?.properties
      ? Object.keys(municipal.features[0].properties)
      : [],
    sample: municipal.features.slice(0, 3).map((f) => f.properties),
  },
  parcels: {
    count: parcels.features.length,
    fields: parcels.features[0]?.properties
  ...
  landuse: {
    ...
    fields: landuse.features[0]?.properties
      ? Object.keys(landuse.features[0].properties)
      : [],
    sample: landuse.features.slice(0, 3).map((f) => f.properties),
  },
};
await writeFile(
  base + "/_neighborhood_stats.json",
